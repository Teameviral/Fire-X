...
@fire.on(fire_on_cmd("lock( (?P<target>\S+)|$)"))
@fire.on(sudo_cmd("lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@fire.on(fire_on_cmd("unlock ?(.*)"))
@fire.on(sudo_cmd("unlock ?(.*)", allow_sudo=True))
async def _(event):
...
@fire.on(fire_on_cmd("curenabledlocks"))
@fire.on(fire_on_cmd("curenabledlocks", allow_sudo=True))
async def _(event):
...
@fire.on(events.MessageEdited())  # pylint:disable=E0602
@fire.on(events.NewMessage())  # pylint:disable=E0602
...
@fire.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
