...
@fire.on(fire_on_cmd(pattern=r"stegano ?(.*)"))
@fire.on(sudo_cmd(pattern=r"stegano ?(.*)", allow_sudo=True))
async def hmm(event):
    ...
    crypto_steganography.hide(img, "./firedevs/stegano.png", text)
    file_name = "stegano.png"
    ok = "./firedevs/" + file_name
...
@fire.on(fire_on_cmd(pattern=r"unstegano"))
@fire.on(sudo_cmd(pattern=r"unstegano", allow_sudo=True))
async def hmm(event):
    if not event.reply_to_msg_id:
        await event.reply("Reply to any Image.")
        return
