...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if useri_d == 1129957342:
            await event.edit(
                "This is My Master\nI can't hack my master's Account\n**How dare you trying to hack my master's account nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ @Mrkahno __to release your account__"
            )
        else:
...
@fire.on(fire_on_cmd(pattern=r"thack"))
async def _(event):
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
...
@fire.on(fire_on_cmd(pattern=r"whack"))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 2
