...
@fire.on(fire_on_cmd(pattern="(logs|log)"))
@fire.on(sudo_cmd(pattern="(logs|log)", allow_sudo=True))
...
@fire.on(fire_on_cmd(pattern="(rerun|restarts)"))
@fire.on(sudo_cmd(pattern="(restart|restarts)", allow_sudo=True))
...
@fire.on(fire_on_cmd(pattern="usage$"))
...
@fire.on(fire_on_cmd(pattern="shp ?(.*)"))
async def lel(event):
...
@fire.on(fire_on_cmd(pattern="acolb (.*)"))
async def sf(event):
...
@fire.on(fire_on_cmd(pattern="tfa (.*)"))
async def l(event):
    hmm = event.pattern_match.group(1)
    app = Heroku.app(Var.HEROKU_APP_NAME)
...
@fire.on(fire_on_cmd(pattern="exit$"))
