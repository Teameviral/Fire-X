...
import asyncio
from firebot.events import friday_on_cmd as lightning_cmd
config=Config
...
GIT_REPO_NAME = "Fire-X"
UPSTREAM_REPO_URL = "https://github.com/Chrisdroid1/Fire-X"
...
async def upstream(ups):
    ...
    if force_updateme:
        ...
        await ups.edit('`Updating your` Fire-X 𝐔𝐬𝐞𝐫𝐛𝐨𝐭 `please wait for a while`')
        await asyncio.sleep(4)
        await ups.edit('Updated Sur✨\nRestarting it please have patience and enjoy botless life for a while\nIncase restart starts join [Fire-X 𝐒𝐮𝐩𝐩𝐨𝐫𝐭](https://t.me/FIRE_X_CHANNEL)')
    ...
    if config.HEROKU_API_KEY is not None:
        ...
        await ups.edit('`Sync Verified Successfully 🙂🙂\n'
                       'Restarting, please wait a min ,then type .alive to check if I alive !! Else Go to [Fire-X 𝐒𝐮𝐩𝐩𝐨𝐫𝐭](https://t.me/FIRE_X_CHANNEL)`')
    else:
